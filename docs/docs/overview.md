...
Our Node SDK exposes HTTP endpoints that accept requests with JSON arguments and return JSON responses. Authentication is done via the API key passed to the `initSalable` function.

Specific versions of the Salable API can also be specified as the second argument of the `initSalable` function.
