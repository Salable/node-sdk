...
function merge(target: Record<string, unknown>, source?: Record<string, unknown>) {
  ...
  for (const key of Object.keys(source)) {
    if (isObject(source[key])) {
      clone[key] = merge(clone[key] as Record<string, unknown>, source[key] as Record<string, unknown>);
    } else {
...
export default function objectBuilder<T>(defaultParameters: T) {
  return (overrideParameters?: DeepPartial<T> | null): T => {
    ...
    return merge(defaultParameters as Record<string, unknown>, overrideParameters as Record<string, unknown>) as T;
  };
}
